...
<template>
  <div class="prose m-auto mb-8 select-none animate-none! op100!">
    <div mb-0 flex="~ col gap-1 sm:row sm:gap-3 wrap" text-3xl>
      <RouterLink to="/links" class="!border-none" :class="route.path === '/links' ? activeStyle : inactiveStyle">
        友链
      </RouterLink>
      ...
      <RouterLink to="/circle" class="!border-none" :class="route.path === '/circle' ? activeStyle : inactiveStyle">
        鱼塘
      </RouterLink>
    </div>
  </div>
